...
A comprehensive Flutter SDK wrapper for Firebase, Mixpanel, Adjust, RevenueCat, App Tracking Transparency, and In-App Review with unified analytics and crash reporting.
...
- P **In-App Review**: Request app store reviews
...
final config = TangentConfig(
  ...
  environment: TangentEnvironment.production, // or .sandbox
...
```dart
await TangentSDK.instance.trackEvent('user_signup', properties: {
  'method': 'email',
  'plan': 'premium',
});
```
...
```dart
await TangentSDK.instance.trackFailureEvent(
...
```dart
await TangentSDK.instance.trackSubscription(
...
#### Get Products
```dart
final result = await TangentSDK.instance.getProducts(['product_id_1', 'product_id_2']);
result.when(
...
#### Purchase Product
```dart
final result = await TangentSDK.instance.purchaseProduct('premium_monthly');
result.when(
  success: (purchaseResult) => print('Purchase: $purchaseResult'),
  failure: (error) => print('Error: $error'),
);
```
...
```dart
final result = await TangentSDK.instance.checkActiveSubscription();
result.when(
...
```dart
TangentSDK.instance.hasActiveSubscriptionStream.listen((hasSubscription) {
  print('Subscription status changed: $hasSubscription');
});
```
...
#### Get Current Status
```dart
final status = await TangentSDK.instance.getTrackingStatus();
print('Tracking status: $status');
```

#### Get Advertising Identifier
```dart
final adId = await TangentSDK.instance.getAdvertisingIdentifier();
print('Ad ID: $adId');
```
...
```dart
final available = await TangentSDK.instance.isReviewAvailable();
if (available) {
  await TangentSDK.instance.requestReview();
}
```
...
#### Open Store Listing
```dart
await TangentSDK.instance.openStoreListing(appStoreId: 'your_app_store_id');
```
...
#### Log Messages
```dart
await TangentSDK.instance.log('User performed action X');
```
...
```dart
final result = await TangentSDK.instance.getProducts(['product_id']);
result.when(
  success: (data) {
    // Handle success
  },
